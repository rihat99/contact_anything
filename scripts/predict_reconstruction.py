...
def _load(checkpoint: Path, config: Path, device: str, kind: str) -> dict:
    ...
    if has_contact:
        if spec.joint_names not in (EXTREMITY_4_NAMES, KINDYN_6_NAMES):
            raise ValueError(
                f"{checkpoint}: reconstruction prediction requires extremities_4 or "
                f"kindyn_6; got {spec.joint_names}")
        anchors = list(model.contact_keypoint_indices)
        if len(anchors) != len(spec.joint_names):
            raise ValueError(
                f"{checkpoint}: expected {len(spec.joint_names)} MHR anchors; "
                f"got {anchors}")
        limb_names = list(spec.joint_names)
