...
training: the same :func:`contact.data.collate._process_sample` transform on
the same bbox, then ``model.data_preprocess`` + backbone in bf16. Mask and ray
conditioning are NOT baked in — they run live downstream of the cache.
...
from contact.config import load_config                                # noqa: E402
from contact.data.climbing_corpus import (                            # noqa: E402
    DEFAULT_ROOT, ClimbingCorpusDataset, embedding_path, list_corpus_scenes)
from contact.data.collate import _build_transform, _process_sample    # noqa: E402
from contact.model import build_model                                 # noqa: E402
...
class CropDataset(Dataset):
    ...
    ``[3, H, W]`` float crop :func:`_process_sample` produces (the mask plays
    no part in the backbone input, so none is loaded).
    """
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, int]:
        ...
        image = np.array(
            ...
            np.uint8)
        frame = {"image": image, "mask": None, "bbox": bbox,
                 "key": f"{scene}#{oid}@{pos}"}
        return _process_sample(frame, self.transform)["img"], index
...
def collect_entries(
    ...
    Returns ``(entries, num_existing, failed_scenes)``. Scene feature loading
    reuses :class:`ClimbingCorpusDataset` (one scene at a time so a scene with
    missing/broken features is skipped, not fatal) — the ``valid_mask`` /
    ``bbox`` the cache keys on are exactly the training loader's.
    """
    entries: list[tuple] = []
    num_existing = 0
    failed: list[str] = []
    for scene in tqdm(scenes, desc="index scenes"):
        try:
            ds = ClimbingCorpusDataset(
                corpus_root, scenes=[scene], split="test", require_labels=False,
...
def compute_entries(
    ...
    *,
    ...
    The exact training input path: :func:`_process_sample` crop ->
    ``model.data_preprocess`` -> backbone in ``model.backbone_dtype``; the raw
    bf16 output is stored as int16 bits. Returns the elapsed seconds.
    """
...
def main() -> None:
    ...
    scenes = [s for split in splits for s in list_corpus_scenes(corpus_root, split)]
