...
from model.sam_3d_body.data.utils.prepare_batch import prepare_batch
from model.sam_3d_body.models.decoders.prompt_encoder import PositionEmbeddingRandom
from model.sam_3d_body.models.modules.mhr_utils import (
    fix_wrist_euler,
    rotation_angle_difference,
)
from model.sam_3d_body.utils import recursive_to
from model.sam_3d_body.utils.logging import get_pylogger
...
# --- extra-token-block hook (imports + types) ---
from dataclasses import dataclass, field
from typing import Callable


@dataclass
class ExtraTokenBlock:
    """One externally-owned block of tokens appended to the body decoder sequence.
    ...
    Attributes:
        ...
        tokens: ``[B, K, C]`` embedded tokens, already expanded to the batch.
        update_fn: optional per-interm-layer callback
            ``fn(start_idx, image_embeddings, token_embeddings, token_augment,
...
class SAM3DBody(BaseModel):
    ...
    def _initialze_model(self):
        ...
        # --- efficiency hook (detach_interm_preds) ---
        # When set, the decoder runs its per-layer interm MHR/camera predictions
        # under no_grad (they feed keypoint/extra-token *sampling locations* only,
    ...
    def forward_decoder(
        ...
        if hasattr(self, "prompt_encoder") and keypoints is not None:
            ...
            # --- extra-token-block hook (append + asymmetric mask) ---
            ...
            if extra_blocks:
                for _blk in extra_blocks:
                    assert _blk.tokens.dim() == 3 and _blk.tokens.shape[0] == batch_size, (
                        f"block {_blk.name!r}: tokens must be [B, K, C] with "
                        f"B={batch_size}; got {tuple(_blk.tokens.shape)}")
                    assert _blk.name not in extra_block_bounds, (
                        f"duplicate extra block name {_blk.name!r}")
                    _start = token_embeddings.shape[1]
                    token_embeddings = torch.cat(
                        [token_embeddings, _blk.tokens], dim=1)
                    token_augment = torch.cat(
        ...
        def keypoint_token_update_fn_comb(*args):
            ...
            # --- extra-token-block hook (per-layer update callbacks) ---
            # Each block owner refreshes its own tokens from this layer's
            # interm pose prediction (e.g. keypoint-anchored feature sampling).
            # The callbacks receive the same ray-conditioned image embeddings
            # the original keypoint update samples from. Skipped after the
            # last layer, like the original update fns.
            if extra_blocks:
                _tok, _aug, _pose_out, _layer_idx = args
                if _layer_idx < len(self.decoder.layers) - 1:
                    for _blk in extra_blocks:
                        if _blk.update_fn is None:
                            continue
                        _tok, _aug = _blk.update_fn(
                            extra_block_bounds[_blk.name][0], image_embeddings,
                            _tok, _aug, _pose_out)
                args = (_tok, _aug, _pose_out, _layer_idx)
            # --- end extra-token-block hook ---
            return args
        ...
        # Callers that passed extra_blocks (even an empty list) get a third
        # value: the FULL final token sequence plus each block's [start, end)
        # bounds, so the owner can slice its tokens (and the pose token) out
        # regardless of the hand-detect return convention below.
        if extra_blocks is not None:
            extra = {"tokens": pose_token, "blocks": extra_block_bounds}
            if self.cfg.MODEL.DECODER.get("DO_HAND_DETECT_TOKENS", False):
                return (
            ...
            return pose_token, pose_output, extra
        # --- end extra-token-block hook ---
    ...
    def forward_pose_branch(
        ...
        Args:
            ...
            precomputed_features: optional ``[B*N, C, H, W]`` backbone
                embeddings (the RAW backbone output). When provided, the
                backbone is skipped entirely; mask and ray conditioning still
                run live, so the decoder sees exactly what the image path
                produces.
        ...
        if precomputed_features is not None:
            # --- efficiency hook (precomputed backbone embeddings) ---
            # The cache stores the RAW backbone output (pre-cast, pre
            # mask-conditioning); this cast reproduces the live path's
            # backbone-dtype -> img-dtype cast bit-exactly. Mask conditioning
            # below and ray conditioning inside forward_decoder still run live.
            if self.cfg.MODEL.BACKBONE.TYPE in [
            ...
            # --- efficiency hook (backbone_no_grad) ---
            # The backbone is fully frozen in this fork's training use, so
            # wrapping ONLY this call in no_grad drops its activation graph
            # without touching any trainable param. Absent key = old behaviour.
        ...
        pose_output, pose_output_hand = None, None
        extra = None                              # extra-token-block hook
        if len(self.body_batch_idx):
            # --- extra-token-block hook (pass blocks through) ---
            if extra_blocks is not None:
                tokens_output, pose_output, extra = self.forward_decoder(
                    image_embeddings[self.body_batch_idx],
                    init_estimate=None,
        ...
        # --- extra-token-block hook (expose sequence + block bounds) ---
        if extra is not None:
            output["tokens"] = extra["tokens"]    # [B, N_total, C] final sequence
            output["blocks"] = extra["blocks"]    # {name: (start, end)}
        # --- end extra-token-block hook ---
