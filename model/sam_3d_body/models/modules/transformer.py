...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if x_mask is not None:
            if x_mask.dim() == 3:
                # Pre-computed asymmetric mask [B, N, N]: used when some tokens (e.g. appended
                # extra tokens) should not be visible to original tokens as keys, while
                # the appended tokens themselves can still attend to all other tokens.
                attn_mask = x_mask
            else:
        ...
        if self.enable_twoway:
            ...
            if x_mask is None:
                ...
                # (i.e. appended extra tokens). Using all(dim=1) gives the intersection of
